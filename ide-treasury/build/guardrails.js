 ...
 */
export function getMissionConfig(taskId) {
 ...
 */
export async function validateSpend(amount, missionId) {
    ...
    if (amount >= USER_APPROVAL_THRESHOLD) {
        console.warn(`[Guardrails] Payment of ${amount.toFixed(2)} USDC exceeds USER_APPROVAL_THRESHOLD ` +
    ...
    if (projectedTotal > mission.maxBudget) {
        const error = new Error(`BUDGET_EXCEEDED_REQUIRES_HUMAN_INTERVENTION: ` +
        ...
        error.code = 'BUDGET_EXCEEDED_REQUIRES_HUMAN_INTERVENTION';
        throw error;
    }
    return true;
}
 ...
 */
export function recordSpend(amount, taskId = 'default') {
 ...
 */
export function getRemainingBudget(taskId = 'default') {
    const mission = getMissionConfig(taskId);
 ...
 */
export function resetTaskBudget(taskId) {
 ...
 */
export function getUserApprovalThreshold() {
    return USER_APPROVAL_THRESHOLD;
}
/**
 * Get the default max budget
 */
export function getDefaultMaxBudget() {
